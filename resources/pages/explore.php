...
<div class="max-w-7xl mx-auto px-6 pt-6 pb-16" x-data="{ activeFilter: 'All' }">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-10">
        <div>
            <h1 class="text-4xl font-extrabold mb-2 text-kallani-text tracking-tight">Project Portfolio</h1>
            <p class="text-lg text-kallani-text-secondary">Explore productive natural assets managed through the Kallani operating layer.</p>
        </div>
        <div class="px-4 py-2 bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 rounded-xl text-xs font-bold border border-emerald-200 dark:border-emerald-800">
    ...
    <div class="mb-10 flex gap-2.5 flex-wrap">
        ...
        <button @click="activeFilter = '<?php echo $cat; ?>'" 
                :class="activeFilter === '<?php echo $cat; ?>' ? 'bg-emerald-700 text-white font-bold shadow-md' : 'bg-kallani-surface text-kallani-text-secondary hover:text-kallani-text hover:bg-kallani-muted'" 
                class="px-5 py-2.5 text-sm rounded-xl transition-all duration-200 border border-kallani-border cursor-pointer">
            <?php echo $cat; ?>
        </button>
        <?php endforeach; ?>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        <div class="card hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 flex flex-col justify-between overflow-hidden group" 
            ...
            <div>
                <!-- Image Box -->
                <div class="relative h-48 -mx-6 -mt-6 mb-5 overflow-hidden bg-kallani-muted">
                ...
                <h3 class="text-xl font-bold mb-2 text-kallani-text group-hover:text-emerald-700 dark:group-hover:text-emerald-400 transition-colors"><?php echo $project['name']; ?></h3>
                
                <div class="space-y-3 mb-6 pb-6 border-b border-kallani-border">
                    <div class="flex gap-2 flex-wrap">
                        <span class="inline-block px-3 py-1 bg-kallani-muted text-kallani-text text-xs font-semibold rounded-md">
                    ...
                    <p class="text-xs font-medium text-kallani-text-secondary">Status: <span class="font-bold text-kallani-text"><?php echo $project['status']; ?></span></p>
                </div>

                <div class="space-y-4 mb-6">
                    <div>
                        <div class="flex justify-between mb-2">
                            <span class="text-xs font-medium text-kallani-text-secondary">GIS Verification</span>
                            <span class="text-xs font-bold text-emerald-600 dark:text-emerald-400"><?php echo $project['verification']; ?>%</span>
                        </div>
                        <div class="w-full bg-kallani-muted rounded-full h-2 overflow-hidden">
                    ...
                    <div class="grid grid-cols-2 gap-4 text-xs p-3 rounded-xl bg-kallani-muted/60 border border-kallani-border/50">
                        <div>
                            <p class="text-kallani-text-secondary">Capital Required</p>
                            <p class="font-bold text-sm text-kallani-text">$<?php echo number_format($project['capital_required'] / 1000000, 1); ?>M</p>
                        </div>
                        <div>
                            <p class="text-kallani-text-secondary">Capital Committed</p>
                            <p class="font-bold text-sm text-emerald-700 dark:text-emerald-400">$<?php echo number_format($project['capital_committed'] / 1000000, 1); ?>M</p>
                        </div>
                    </div>
                </div>
            </div>
